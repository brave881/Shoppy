package com.brave.shoppy.screen.payment.utils
    ...
    companion object {
        ...
        */
        ...
        private fun isValidCardNumberLength(inputCardNumber: String): Boolean {
            val result: Boolean =
                ((inputCardNumber.length >= CARD_NUMBER_LENGTH_MIN) && (inputCardNumber.length <= CARD_NUMBER_LENGTH_MAX))

            printLog("isValidCardNumber Length () = $result")
            return result
        }

        private fun isValidCardNumberValue(inputCardNumber: Long): Boolean = (inputCardNumber > 0)
        ...
        fun isValidCard(
            inputCardNumber: Long, inputCVC: Int, inputExpMonth: Int, inputExpYear: Int
        ): Boolean =
            (isValidCardNumber(inputCardNumber) && isValidCVC(inputCVC) && isValidExpirationDate(
                inputExpMonth, inputExpYear
            ))
        ...
        * */
        fun isValidCardNumber(inputCardNumber: Long): Boolean {
            ...
            return isValidCardNumberValue(inputCardNumber) && isValidCardNumberLength(
                inputCardNumber.toString()
            ) && isValidCardNumberByTypeSupport(inputCardNumber.toString()) && isValidCardNumberByLuhn(
                inputCardNumber.toString()
            )
        }
        ...
        * */
        fun isValidCVC(inputCVC: Int): Boolean {
            ...
            val result: Boolean =
                ((stringInputCVC.length >= CVC_LENGTH_MIN) && (stringInputCVC.length <= CVC_LENGTH_MAX))
        ...
        * */
        fun isValidExpirationDate(inputExpMonth: Int, inputExpYear: Int): Boolean {
            ...
            val result =
                ((isValidMonthRange && isValidYearLength && isValidYearValue) && (isFutureYear || isSameYear_FutureOrCurrentMonth))
